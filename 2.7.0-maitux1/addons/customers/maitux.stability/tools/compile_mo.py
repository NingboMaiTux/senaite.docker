# -*- coding: utf-8 -*-
"""将 maitux.stability 的 .po 编译为 .mo（gettext 二进制格式）
...
为什么必须有这个脚本：仓库根 `.gitignore` 里 `*.mo` 是不跟踪的（.po 才是源），
而本环境**没有**开启 zope.i18n 的自动编译（`zope_i18n_compile_mo_files` 在
compose / Dockerfile / buildout 里都没设），容器启动时不会从 .po 重建 .mo。
所以：**磁盘上一定要有编译好的 .mo，改了 .po 就要在这里重跑一遍**。
"""
import os
import struct
