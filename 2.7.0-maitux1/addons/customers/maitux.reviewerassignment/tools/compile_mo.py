# -*- coding: utf-8 -*-
"""将 maitux.reviewerassignment 的 .po 编译为 .mo（gettext 二进制格式）
...
为什么自己写：容器和宿主都没有 `msgfmt`（实测容器内 `which msgfmt` 为空），
Python 也不提供可 import 的 msgfmt 模块。GNU 的 .mo 格式本身很简单，
自己生成比引入新依赖划算。与 maitux.worksheetfields / maitux.auditjournal
里的同名脚本是同一套做法。
