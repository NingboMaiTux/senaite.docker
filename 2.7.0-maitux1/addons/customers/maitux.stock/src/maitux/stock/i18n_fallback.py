...
保证 ``maitux.stock`` 域总是被额外查询（对其它附加域/Message 行为无影响）。
"""
...
def install():
    ...
    if original is None:
        return
    if getattr(original, "_maitux_stock_i18n", False):
    ...
    translate._maitux_stock_i18n = True
