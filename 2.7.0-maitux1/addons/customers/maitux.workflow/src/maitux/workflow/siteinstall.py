...
是站点级的。两者不挂钩，所以 ``guards/configure.zcml`` 里那个 ``for="*"`` 的
IGuardAdapter 会在**所有**站点参与每一次工作流 guard 求值，包括从没装过
maitux.workflow 的站点。

``bika.lims.workflow.guard_handler`` 用 ``getAdapters((instance,), IGuardAdapter)``
遍历所有适配器，**任一返回 False 即否决整个 transition** —— 也就是说写错一行就能
把未安装站点的原生工作流按钮拦掉，而且现象是"按钮没了"，不报错。

IGuardAdapter 的查找签名只有 ``(instance,)``，没有 request，所以 ZCML 的
``layer=`` 在这里用不上（那只对 ``(context, request)`` 的多适配器有效）。
只能在适配器内部做站点级判断，未安装则放行。

本模块是 ``maitux.reviewerassignment.siteinstall`` 的同构实现。**刻意复制而不是
import**：两个包之间不建立依赖（CLAUDE.md §5「addon 之间不要反向依赖」），
其中一个被卸载不应影响另一个。
"""
...
PRODUCT_NAME = "maitux.workflow"
PROFILE_ID = "maitux.workflow:default"
...
REQUEST_CACHE_KEY = "maitux.workflow.site_installed"
...
def query_site_installed(site):
    ...
    也是判断「这个站点要不要本 addon」最贴切的语义。取不到再退回 portal_setup
    的 profile 版本。
    """
    if site is None:
        return False
