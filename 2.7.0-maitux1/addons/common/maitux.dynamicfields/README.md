...
配置项按字段类型显隐，每块用 `data-mdf-types` 声明适用类型，一小段原生 JS
按当前选中的类型切换：

| 配置块 | 适用类型 |
|---|---|
| 选项列表 | `choice` |
| 引用目标（允许的类型、含停用对象） | `reference` |
| 多值 | `choice` `reference` |
...
- **没有 JS 时全部显示**，功能不受影响，只是表单长一点
- **服务端不依赖界面显隐**。`_read_type_specific()` 本来就只按 `field_type`
  读对应项；`multi` 也已收口成只对可多值类型生效——藏起来的输入框照样会被
  提交，构造请求更是绕得过
...
**完整保存路径吃 bytes 中文**、**模板表单完整性**（视图要读的 31 个输入框一个不能少、
模板引用的视图方法必须真实存在、`data-mdf-types` 里的类型 id 必须合法、
除 date/datetime 外每种类型都要有专属配置块）。
